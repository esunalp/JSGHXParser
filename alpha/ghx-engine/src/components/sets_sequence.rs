...
mod tests {
    ...
    fn test_range() {
        ...
        assert_eq!(list[10], Value::Number(10.0));
    }
    ...
    fn test_fibonacci() {
        ...
        let outputs = fib.evaluate(&[a, b, n], &MetaMap::new()).unwrap();
        let sequence = match outputs.get("S") {
            Some(Value::List(seq)) => seq,
            _ => panic!("Expected a list output for 'S'"),
        };

        let expected: Vec<Value> = vec![0.0, 1.0, 1.0, 2.0, 3.0, 5.0, 8.0, 13.0, 21.0, 34.0]
            .into_iter()
            .map(Value::Number)
            .collect();
    ...
    #[test]
    fn test_cull_pattern() {
        let cull = CullPattern;
        ...
        let outputs = cull.evaluate(&[list, pattern], &MetaMap::new()).unwrap();
    ...
    fn test_cull_nth() {
        ...
        let outputs = cull.evaluate(&[list, n], &MetaMap::new()).unwrap();
        let culled_list = match outputs.get("L").unwrap() {
            Value::List(l) => l,
            _ => panic!("Expected a list"),
        };
    ...
    fn test_random() {
        let random = Random;
        let range = Value::Domain(Domain::One(create_domain(0.0, 100.0).unwrap()));
    ...
    #[test]
    fn test_random_reduce() {
        let reduce = RandomReduce;
        let list = Value::List(vec![
            Value::Number(1.0),
            Value::Number(2.0),
            Value::Number(3.0),
        ...
        let reduction = Value::Number(2.0);
        let seed = Value::Number(123.0);
        let outputs = reduce
            .evaluate(&[list, reduction, seed], &MetaMap::new())
            .unwrap();
        let reduced_list = match outputs.get("L").unwrap() {
            Value::List(l) => l,
            _ => panic!("Expected a list"),
        };
        assert_eq!(reduced_list.len(), 3);
    }
    ...
    fn test_stack_data() {
        ...
        let pattern = Value::List(vec![Value::Number(2.0), Value::Number(3.0)]);
        let outputs = stack.evaluate(&[data, pattern], &MetaMap::new()).unwrap();
        let stacked_list = match outputs.get("D").unwrap() {
            Value::List(l) => l,
            _ => panic!("Expected a list"),
        };
        assert_eq!(
    ...
    #[test]
    fn test_repeat_data() {
        let repeat = RepeatData;
        let data = Value::List(vec![Value::Number(1.0), Value::Number(2.0)]);
        let length = Value::Number(5.0);
        let outputs = repeat.evaluate(&[data, length], &MetaMap::new()).unwrap();
        let repeated_list = match outputs.get("D").unwrap() {
            Value::List(l) => l,
            _ => panic!("Expected a list"),
        };
        assert_eq!(
    ...
    fn test_duplicate_data() {
        ...
        let outputs_ordered = duplicate
            .evaluate(&[data, number, Value::Boolean(true)], &MetaMap::new())
            .unwrap();
        let duplicated_list_ordered = match outputs_ordered.get("D").unwrap() {
            Value::List(l) => l,
            _ => panic!("Expected a list"),
        };
        assert_eq!(
            *duplicated_list_ordered,
            vec![
                Value::Number(1.0),
                Value::Number(1.0),
                Value::Number(2.0),
    ...
    fn test_jitter() {
        ...
        let indices = match outputs.get("I").unwrap() {
            Value::List(l) => l,
            _ => panic!("Expected a list"),
        };
