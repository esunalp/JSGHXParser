...
mod tests {
    ...
    #[test]
    fn project_flattens_points() {
        let component = ComponentKind::Project;
        let outputs = component
            .evaluate(
                &[Value::Point([1.0, 2.0, 3.0]), plane_xy()],
                &MetaMap::new(),
            )
            .expect("project");
    ...
    #[test]
    fn scale_expands_distance() {
        let component = ComponentKind::Scale;
        let outputs = component
            .evaluate(
                &[
                    Value::Point([1.0, 0.0, 0.0]),
        ...
        assert!((point[0] - 2.0).abs() < 1e-6);
    }

    #[test]
    fn scale_non_uniform_respects_axes() {
        let component = ComponentKind::ScaleNonUniform;
        let outputs = component
            .evaluate(
                &[
        ...
        assert!((point[0] - 2.0).abs() < 1e-6);
        assert!((point[1] - 3.0).abs() < 1e-6);
        assert!((point[2] - 4.0).abs() < 1e-6);
    }

    #[test]
    fn triangle_mapping_preserves_corners() {
        let component = ComponentKind::TriangleMapping;
        ...
        let outputs = component
            ...
            .expect("triangle mapping");
        let Value::List(values) = outputs
            .get(super::PIN_OUTPUT_GEOMETRY)
            .cloned()
            .expect("geometry output")
        else {
        ...
        let Value::Point(mapped) = &values[0] else {
    ...
    #[test]
    fn orient_direction_rotates_point() {
        let component = ComponentKind::OrientDirection;
        let outputs = component
            .evaluate(
                &[
                    Value::Point([1.0, 0.0, 0.0]),
    ...
    #[test]
    fn project_along_uses_direction() {
        let component = ComponentKind::ProjectAlong;
        let outputs = component
            .evaluate(
                &[
                    Value::Point([0.0, 0.0, 0.0]),
