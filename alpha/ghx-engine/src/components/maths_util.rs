...
mod tests {
    use super::{
        Component, ComponentKind, PIN_OUTPUT_ARGUMENT, PIN_OUTPUT_IMAGINARY, PIN_OUTPUT_MEAN,
        PIN_OUTPUT_MODULUS, PIN_OUTPUT_NUMBERS, PIN_OUTPUT_REAL, PIN_OUTPUT_VALUE,
    };
    use crate::graph::node::MetaMap;
    use crate::graph::value::{ComplexValue, Value};

    #[test]
    fn maximum_returns_greater_value() {
        let component = ComponentKind::Maximum;
        let outputs = component
            .evaluate(&[Value::Number(5.0), Value::Number(8.0)], &MetaMap::new())
            .expect("Maximum slaagt");
        assert_eq!(outputs.get(super::PIN_RESULT), Some(&Value::Number(8.0)));
    }

    #[test]
    fn weighted_average_combines_values_and_weights() {
        let component = ComponentKind::WeightedAverage;
        let values = Value::List(vec![Value::Number(1.0), Value::Number(3.0)]);
        let weights = Value::List(vec![Value::Number(1.0), Value::Number(3.0)]);
        let outputs = component
            .evaluate(&[values, weights], &MetaMap::new())
            .expect("Weighted Average slaagt");
        assert!(matches!(
            outputs.get(PIN_OUTPUT_MEAN),
            Some(Value::Number(value)) if (*value - 2.5).abs() < 1e-9
        ));
    }

    #[test]
    fn blur_numbers_applies_smoothing() {
        let component = ComponentKind::BlurNumbers;
        ...
        let outputs = component
            ...
            .expect("Blur Numbers slaagt");
        if let Some(Value::List(values)) = outputs.get(PIN_OUTPUT_NUMBERS) {
            assert_eq!(values.len(), 3);
            assert!(matches!(values[1], Value::Number(value) if value < 10.0 && value > 0.0));
        } else {
            panic!("Blur Numbers leverde geen lijst op");
        }
    }

    #[test]
    fn complex_components_split_complex_number() {
        let component = ComponentKind::ComplexComponents;
        let complex = Value::Complex(ComplexValue::new(4.0, -2.0));
        let outputs = component
            .evaluate(&[complex], &MetaMap::new())
            .expect("Complex Components slaagt");
        assert_eq!(outputs.get(PIN_OUTPUT_REAL), Some(&Value::Number(4.0)));
        assert_eq!(
            outputs.get(PIN_OUTPUT_IMAGINARY),
            Some(&Value::Number(-2.0))
        );
    }

    #[test]
    fn complex_argument_and_modulus_use_helpers() {
        let argument = ComponentKind::ComplexArgument
            .evaluate(
                &[Value::Complex(ComplexValue::new(0.0, 1.0))],
                &MetaMap::new(),
            )
            .expect("Argument slaagt");
        let modulus = ComponentKind::ComplexModulus
            .evaluate(
                &[Value::Complex(ComplexValue::new(3.0, 4.0))],
                &MetaMap::new(),
            )
            .expect("Modulus slaagt");
    ...
    #[test]
    fn interpolate_data_blends_between_values() {
        let component = ComponentKind::InterpolateData;
        let values = Value::List(vec![Value::Number(0.0), Value::Number(10.0)]);
        let outputs = component
            .evaluate(&[values, Value::Number(0.25)], &MetaMap::new())
            .expect("Interpolate slaagt");
        assert_eq!(outputs.get(PIN_OUTPUT_VALUE), Some(&Value::Number(2.5)));
    }
}
