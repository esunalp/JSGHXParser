...
/// # Returns
///
/// A tuple containing two new `Vec<[f64; 3]>`s, representing the resampled
...
pub fn resample_polylines(
    ...
    let mut resampled_shorter = Vec::with_capacity(longer.len());
    let shorter_len = (shorter.len() - 1) as f64;
    let longer_len = (longer.len() - 1) as f64;

    for i in 0..longer.len() {
        let t = i as f64 / longer_len; // Normalized position [0, 1] along the longer polyline
