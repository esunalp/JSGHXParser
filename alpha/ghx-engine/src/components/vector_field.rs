...
mod tests {
    use super::{
        Component, ComponentKind, PIN_OUTPUT_CURVE, PIN_OUTPUT_DISPLAY, PIN_OUTPUT_FIELD,
        PIN_OUTPUT_FIELDS, PIN_OUTPUT_STRENGTH, PIN_OUTPUT_TENSOR,
    };
    use crate::graph::node::MetaMap;
    use crate::graph::value::Value;

    #[test]
    fn point_charge_generates_field() {
        let component = ComponentKind::PointCharge;
        let outputs = component
            ...
            .expect("point charge evaluates");
        assert!(
            matches!(outputs.get(PIN_OUTPUT_FIELD), Some(Value::List(entries)) if !entries.is_empty())
        );
    }

    #[test]
    fn evaluate_field_reports_strength() {
        let field = ComponentKind::PointCharge
            .evaluate(
                &[Value::Point([0.0, 0.0, 0.0]), Value::Number(3.0)],
                &MetaMap::new(),
            )
            .expect("field created")
            .remove(PIN_OUTPUT_FIELD)
            .expect("field output present");

        let outputs = ComponentKind::EvaluateField
            .evaluate(&[field, Value::Point([1.0, 0.0, 0.0])], &MetaMap::new())
            .expect("evaluation succeeds");

        assert!(
            matches!(outputs.get(PIN_OUTPUT_TENSOR), Some(Value::List(values)) if values.len() == 5)
        );
        assert!(
            matches!(outputs.get(PIN_OUTPUT_STRENGTH), Some(Value::Number(value)) if *value > 0.0)
        );
    }

    #[test]
    fn merge_and_break_fields_works() {
        let first = ComponentKind::PointCharge
            .evaluate(&[Value::Point([0.0, 0.0, 0.0])], &MetaMap::new())
            .expect("first field")
            .remove(PIN_OUTPUT_FIELD)
            .unwrap();
        let second = ComponentKind::PointCharge
            .evaluate(&[Value::Point([1.0, 0.0, 0.0])], &MetaMap::new())
            .expect("second field")
            .remove(PIN_OUTPUT_FIELD)
            .unwrap();

        let merged = ComponentKind::MergeFields
            .evaluate(
                &[Value::List(vec![first.clone(), second.clone()])],
                &MetaMap::new(),
            )
        ...
        let outputs = ComponentKind::BreakField
            .evaluate(&[merged], &MetaMap::new())
            .expect("break succeeds");

        assert!(
            matches!(outputs.get(PIN_OUTPUT_FIELDS), Some(Value::List(entries)) if entries.len() == 2)
        );
    }

    #[test]
    fn tensor_display_samples_points() {
        let field = ComponentKind::PointCharge
            .evaluate(&[Value::Point([0.0, 0.0, 0.0])], &MetaMap::new())
            .expect("field")
            .remove(PIN_OUTPUT_FIELD)
        ...
        let outputs = ComponentKind::TensorDisplay
            .evaluate(
                &[
                    field,
                    Value::List(vec![
            ...
            .expect("display evaluates");

        assert!(
            matches!(outputs.get(PIN_OUTPUT_DISPLAY), Some(Value::List(entries)) if entries.len() == 9)
        );
    }

    #[test]
    fn field_line_returns_polyline() {
        let field = ComponentKind::PointCharge
            .evaluate(&[Value::Point([0.0, 0.0, 0.0])], &MetaMap::new())
            .expect("field")
            .remove(PIN_OUTPUT_FIELD)
        ...
        let outputs = ComponentKind::FieldLine
            .evaluate(
                &[
                    field,
        ...
        assert!(
            matches!(outputs.get(PIN_OUTPUT_CURVE), Some(Value::List(points)) if points.len() > 1)
        );
    }
}
